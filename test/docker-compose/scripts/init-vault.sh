...
PKI_ROLE="${VAULT_PKI_ROLE:-grpc-server}"
CERT_TTL="${CERT_TTL:-8760h}"
SERVER_CN="${SERVER_CN:-grpc-server}"
...
create_roles() {
  ...
  echo "==> Creating PKI role 'grpc-client' for client certificates ..."
  vault write "${PKI_PATH}/roles/grpc-client" \
    allowed_domains="grpc-client,localhost,test-client" \
    allow_subdomains=true \
    allow_bare_domains=true \
    allow_localhost=true \
    allow_ip_sans=true \
...
issue_client_cert() {
  ...
  vault write -format=json "${PKI_PATH}/issue/grpc-client" \
    common_name="${cn}" \
    ttl="${CERT_TTL}" \
    > "/tmp/${filename}.json"
