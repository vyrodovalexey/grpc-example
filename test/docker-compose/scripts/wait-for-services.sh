...
#
# Verifies that Vault PKI is configured, Keycloak realm exists, and the gRPC
...
#   GRPC_HOST        – gRPC host     (default: localhost)
...
# ---------------------------------------------------------------------------
# Check 5 – gRPC server is accepting TCP connections
# ---------------------------------------------------------------------------
check_grpc_server() {
  check_with_timeout "gRPC server (${GRPC_HOST}:${GRPC_PORT})" \
    "nc -z ${GRPC_HOST} ${GRPC_PORT} 2>/dev/null || (echo > /dev/tcp/${GRPC_HOST}/${GRPC_PORT}) 2>/dev/null" \
    2
}
...
main() {
  ...
  check_grpc_server || failed=$((failed + 1))
