...
REALM="${REALM_NAME:-grpc-test}"
CLIENT_ID="${CLIENT_ID:-grpc-server}"
CLIENT_SECRET="${CLIENT_SECRET:-grpc-server-secret}"
...
create_realm() {
  ...
  http_post_json \
    ...
    "{\"realm\": \"${REALM}\", \"enabled\": true, \"displayName\": \"gRPC Test Realm\", \"sslRequired\": \"none\"}" \
    > /dev/null
...
ensure_client() {
  ...
  http_post_json \
    ...
    "{\"clientId\": \"${CLIENT_ID}\", \"name\": \"gRPC Server\", \"enabled\": true, \"clientAuthenticatorType\": \"client-secret\", \"secret\": \"${CLIENT_SECRET}\", \"protocol\": \"openid-connect\", \"publicClient\": false, \"serviceAccountsEnabled\": true, \"standardFlowEnabled\": true, \"directAccessGrantsEnabled\": true, \"redirectUris\": [\"*\"], \"webOrigins\": [\"*\"]}" \
    > /dev/null
...
ensure_roles() {
  ...
  for role in "grpc:read" "grpc:write" "admin"; do
...
main() {
  ...
  ensure_roles
  ensure_client
  ensure_user "test-user" "test-password" "test-user@example.com" "grpc:read"
  ensure_user "admin-user" "admin-password" "admin-user@example.com" "grpc:read" "grpc:write" "admin"
  ...
  echo " Users:         test-user (grpc:read), admin-user (grpc:read, grpc:write, admin)"
}
